...
public sealed class WindowsMachineHealthProbe : IMachineHealthProbe
{
    ...
    public IReadOnlyList<HealthCheckResult> Run() => [
        Guard("defender.realtime", "Defender real-time protection", CheckDefenderRealtime),
        Guard("defender.tamper-protection", "Tamper Protection", CheckTamperProtection),
        Guard("boot.secure-boot", "Secure Boot", CheckSecureBoot),
        Guard("boot.tpm", "TPM present and ready", CheckTpm),
        Guard("bitlocker.os-volume", "BitLocker on the OS volume", CheckBitLocker),
        Guard("vbs.device-guard", "Virtualisation-based security", CheckVbs),
        Guard("lsa.ppl", "LSA protection (RunAsPPL)", CheckLsaPpl),
        Guard("dma.kernel", "Kernel DMA protection", CheckKernelDma),
        Guard("update.build-support", "Windows build in support", CheckBuildSupport),
        Guard("accounts.local-admins", "Local administrators", CheckLocalAdmins),
        Guard("identity.join-state", "Device join state", CheckJoinState),
        Guard("identity.windows-hello", "Windows Hello enrolment", CheckWindowsHello),
        Guard("firewall.profiles", "Firewall enabled on all profiles", CheckFirewall),
        Guard("remote.rdp", "Remote Desktop exposure", CheckRdp),
        Guard("smb.hardening", "SMB hardening", CheckSmb),
        Guard("services.remote-management", "Remote-management services", CheckRemoteServices),
        Guard("apps.sudo-dev-mode", "Sudo and Developer Mode", CheckSudoDevMode),
        Guard("apps.control-policy", "Smart App Control / WDAC", CheckAppControl),
        Guard("drivers.blocklist", "Vulnerable-driver blocklist", CheckDriverBlocklist),
        Guard("core-isolation.hvci", "Core isolation (HVCI)", CheckCoreIsolation),
        Guard("edition.support", "Supported Windows edition", CheckEdition),
        Guard("recovery.winre", "Windows Recovery Environment", CheckWinRe),
        Guard("network.exposure", "Host/port-proxy exposure", CheckNetworkExposure),
        Guard("eventlog.security-size", "Security event-log size", CheckEventLog),
        Guard("defender.exclusions", "Defender exclusions", CheckDefenderExclusions),
        Guard("policy.local-security", "Local security policy snapshot", CheckLocalSecurityPolicy),
        Guard("av.third-party", "Registered antivirus products", CheckThirdPartyAv),
        Guard("management.enrollment", "MDM enrolment", CheckMdmEnrollment),
    ];
