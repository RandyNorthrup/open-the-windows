...
| Q3 | Publish framework-dependent builds too (small download, needs .NET 10 Desktop Runtime) in addition to self-contained? | Resolved (M7) | Yes: `build/package.ps1` publishes a framework-dependent x64 ZIP (~18 MB) alongside the self-contained ZIPs and MSI(s), which stay self-contained. |
| Q4 | Winget publication account / manifest PR ownership. | Resolved (M7) | Manifest id `RandyNorthrup.OpenTheWindows`, generated by `build/winget-manifest.ps1`; the release workflow prints submission instructions (wingetcreate / a PR to microsoft/winget-pkgs) in the job summary. The actual community-repo submission is a manual owner step per release. |
...
A WiX 5 per-machine MSI (`installer/`, SDK-style, kept out of the solution — it
harvests a published payload) installs the app and CLI under
`%ProgramFiles%\Open the Windows\{app,cli}`, with a Start-menu shortcut, an
optional PATH entry, and the Event Log source registered by a deferred custom
action running the new `otw eventlog install` command. A major upgrade replaces
in place; silent install is `/qn`; the audit trail under
`%ProgramData%\OpenTheWindows` is always preserved on uninstall (no purge switch,
by security decision). `build/package.ps1` produces self-contained portable ZIPs
(x64 + arm64), the MSI(s), a framework-dependent x64 ZIP, a CycloneDX SBOM and
`SHA256SUMS.txt`; `build/winget-manifest.ps1` generates the winget manifest
(validated with `winget validate`). `.github/workflows/release.yml` gates,
packages, attests every asset (`actions/attest-build-provenance`) and publishes a
GitHub Release on a `v*` tag. WiX 5 is used because WiX 6/7 require the paid OSMF
EULA (WIX7015); binaries stay unsigned (ADR 0003) with hashes, SBOM and
attestations as provenance. Install/upgrade/uninstall verified on the Win 11 Pro
25H2 VM (22/22). Certified in
[docs/certification/M7.md](docs/certification/M7.md); channels and verification in
[docs/install.md](docs/install.md); re-sign/allow-list in
[docs/enterprise.md](docs/enterprise.md). Honest deviations: WiX 5 not 7,
